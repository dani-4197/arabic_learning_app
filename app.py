...
def create_test_cards():
    ...
    try:
        ...
        if cards_created > 0:
            flash(f'✓ Created {cards_created} test flashcards! Go to Review to start learning.', 'success')
        else:
            flash('Flashcards already exist for this vocabulary.', 'info')
            
        return redirect(url_for('dashboard'))
        
    except Exception as e:
        flash(f'Error creating flashcards: {e}', 'error')
        print(f"Error in create_test_cards: {e}")
        return redirect(url_for('dashboard'))
    finally:
        conn.close()
